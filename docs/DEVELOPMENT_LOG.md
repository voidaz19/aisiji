...
## 2026-09-03：卡片视图改用 `#` Supertag 补全入口

### 用户确认与目标

- 用户确认以节点内容中的 `#` 浮窗选择 `#卡片视图`，替代页面右侧“添加 Canvas”按钮。
- 卡片视图继续沿用现有 `canvas` 稳定 supertag ID；用户可见名称统一为“卡片视图”。

### 本轮施工

- 新增 CodeMirror Supertag 补全源与输入触发器：仅在词边界触发，避免抢占行首 `# ` 标题语法和词内井号。
- 选择候选时先消费触发文本，再通过编辑器 Facet 调用 Store 的 `addSupertag`；标签不写入 Markdown，重复标签不再出现在候选中。
- InlineEditor 与 GhostEditor 共用同一语义入口；Ghost 节点按现有草稿交接流程先实体化，再附加 supertag。
- 节点内容后新增可见 `#卡片视图` 标签；悬停或键盘聚焦时以叉号替换井号，点击标签通过 Store 移除对应 supertag。卡片视图标题旁同步显示该标签。
- 修复添加卡片视图后页面宽度跳变：Canvas 根视图与普通笔记根视图统一使用 `860px` 内容列宽，避免 AppShell 切换视图时产生布局突变。
- 移除 NotebookPanel 根节点右侧添加按钮，并隐藏节点行上的“+”插入按钮；操作浮窗及其 `Ctrl` 单键入口保留。移除卡片视图页面标题右侧按钮，统一通过标题旁标签移除；代码内部 Canvas 命名保留，界面文案改为“卡片视图”。
- 更新卡片视图 SPEC、Markdown/编辑规范和项目结构索引；README 暂不更新，等待用户验收。

### 验证

- 已通过：定向 Markdown/卡片视图测试（25 条）与生产构建。
- 已通过：完整 `npm run check`（45 个测试文件/343 条测试）。
- 宽度修复后已再次通过 TypeScript 与生产构建检查。

## 2026-09-03：卡片视图改动验收与文档收尾

- 用户确认本轮及此前相关改动验收无误，保留现有实现、测试和 SPEC 记录并准备提交。
- 按验收结果同步更新 README：补充 `#卡片视图` Supertag 补全、节点后的可移除标签、根节点与树内局部卡片网格，以及隐藏 `+` 后通过 `Ctrl` 打开操作浮窗的入口说明。
- README 文档变更通过 `git diff --check`；此前完整 `npm run check` 已通过。

## 2026-09-03：拖拽释放时层级线位置同步修复

### 用户反馈与根因

- 用户指出上一版松手时层级线看起来瞬间出现在原位，无法确认它是否与节点一起连续回位。
- 复核确认上一版释放逻辑只对树内层级线做了透明度动画，没有记录拖拽覆盖层中的层级线坐标，也没有执行从覆盖层位置到树内落点的位置过渡。

### 本轮施工

- 松手前测量拖拽覆盖层中每条被拖子树层级线相对于树容器的实际坐标。
- 松手后让树内对应层级线以同一树布局动画时长，从覆盖层坐标平移到当前落点坐标，并同步从当前距离透明度恢复到目标透明度。
- 层级线目标路径继续由现有 `useHierarchyGuides` 和树布局 FLIP 逻辑提供，未改变层级线命中、折叠或拖拽落点语义。

### 验证

- 已通过：定向拖拽预览测试、完整 `npm run check` 与 `git diff --check`；全量前端测试 45 个文件/340 条通过，构建、模块边界、性能类型和 Rust 检查通过。

## 2026-09-03：拖拽预览验收与 editor chunk 说明

### 用户确认

- 用户确认此前涉及的拖拽修复均验收无误：原始树层级缩进、Canvas 卡片网格预览、Markdown/图片 Live Preview、层级线跟随、距离透明度和释放回位动画均符合预期。

### 构建产物说明

- `dist/assets/editor-JX88pgN-.js` 约 526 kB（gzip 约 182 kB），主要原因是 `vite.config.ts` 的 `manualChunks` 将所有 `@codemirror/*` 与 `@lezer/*` 统一集中到 `editor` chunk。
- 该 chunk 包含 CodeMirror 编辑器核心、视图、状态、命令、自动补全、Markdown 语言包，以及 Lezer Markdown/通用语法解析器；应用还在这些基础上加载 Markdown Live Preview、语法树装饰、节点链接补全和附件预览所需逻辑。
- 这是构建拆分策略造成的单 chunk 体积提示，不代表拖拽预览把 Markdown 源文或图片资源打包进该文件；当前 gzip 传输体积约 182 kB。
- 本轮只记录原因，不改变已验收的构建拆分策略，避免引入新的加载时序和回归风险。

### 验证

- 已通过：完整 `npm run check`；保留既有 editor chunk 超过 500 kB 的 Vite 提示。

## 2026-09-03：拖拽层级线透明度跟随修复

### 用户反馈与根因

- 用户指出拖拽时不应生硬隐藏原位置的层级线，层级线应和被拖内容一样，按拖拽距离动态改变透明度。
- 原实现只通过拖拽源占位行的透明度处理内容，层级线没有复用这套距离状态，因此容易出现线条突然消失或与内容状态不一致。

### 本轮施工

- 保留原树中的层级线 DOM，不再过滤或硬隐藏被拖子树的层级线。
- 新增纯函数 `applyDragPreviewGuideOpacity`，仅对被拖子树对应的原位层级线乘以当前拖拽距离透明度；其他节点的层级线保持原有透明度。
- 拖拽距离透明度沿用现有占位内容规则：起始位置为 `0`，随距离渐变，达到阈值后封顶为 `0.3`；拖拽覆盖层中的层级线仍随预览一起移动。
- 松手后复用树布局动画时长，对原位层级线执行从当前透明度到目标透明度的释放过渡，避免内容开始回位时层级线瞬间恢复为满不透明度。
- 补充“起始透明”“封顶透明”“其他层级线不受影响”的回归测试。

### 验证

- 已通过：定向拖拽预览测试、完整 `npm run check` 与 `git diff --check`；全量前端测试 45 个文件/340 条通过，构建、模块边界、性能类型和 Rust 检查通过。

## 2026-09-03：节点拖拽时层级线跟随修复

### 用户反馈与根因

- 用户指出拖拽节点时层级线没有一起移动，按树结构语义，拖动的子树内部层级线应与节点预览保持一致。
- 根因是节点使用 dnd-kit `DragOverlay` 渲染，而层级线仍固定在原 `tree-list` 的 SVG 中；原层级线既不会跟随，也会在拖拽预览旁保留静止的重复视觉。

### 本轮施工

- 拖拽开始时捕获被拖子树内部的层级线，并将树列表坐标转换为拖拽覆盖层坐标。
- 拖拽覆盖层新增 SVG 层级线；原树只隐藏被拖子树内部的线，父级与其他节点共享的外部层级线继续留在原位。
- Canvas 节点的卡片网格预览与其内部层级线一起进入拖拽覆盖层；未改变 Canvas 数据模型、落点语义或非目标约束。
- 新增层级线坐标转换测试，项目索引沿用既有拖拽预览条目；README 等待用户验收后更新。

### 验证

- 待执行：定向层级线/拖拽预览测试与完整 `npm run check`。

## 2026-09-03：Canvas 拖拽预览缩进与卡片视图修复

### 用户反馈与根因

- 用户反馈拖拽时节点缩进丢失；拖拽 Canvas 节点时不应显示为普通大纲视图。
- 根因是拖拽预览把根节点深度归零，只使用子树内部相对层级；同时预览来源仍使用完整 `visibleNodes`，会把局部 Canvas 的卡片后代重新展开成普通大纲行。

### 本轮施工

- 拖拽预览保留拖拽根节点在当前树中的实际深度，再叠加每个后代的相对层级，恢复根节点和子节点的视觉缩进。
- 拖拽来源改用实际渲染行序列，局部 Canvas 的隐藏后代不再进入普通大纲预览。
- Canvas 节点在拖拽覆盖层中保留其卡片网格，并复用只读 Markdown Live Preview 渲染卡片内容；卡片预览禁用交互。
- 回归测试覆盖根节点缩进、子节点缩进和 Canvas 卡片网格视图；README 仍等待用户验收后更新。

### 验证

- 定向测试通过：3 个文件/60 条测试，覆盖根节点原始缩进、子节点相对缩进和 Canvas 卡片网格预览。
- 完整 `npm run check` 通过：模块边界、性能 TypeScript、44 个测试文件/336 条测试、生产构建、Rust 格式和 Rust 编译均正常；Vite 仍只有既有 editor chunk 超过 500 kB 的提示。
- 本地浏览器页面冒烟正常；README 等待用户验收后更新。

## 2026-09-03：Canvas 节点拖拽预览稳定性修复

### 用户反馈与根因

- 用户反馈在 Canvas 视图拖拽带子节点的节点时，子节点会暂时错位；拖拽中的图片、Markdown 语法等内容显示为原文。
- 根因是拖拽覆盖层使用原始 `node.depth` 和源行的绝对 `padding-left`，子树根节点不是从覆盖层自身的零层级开始；覆盖层内容同时直接输出 `node.markdown`，绕过了既有 CodeMirror Live Preview。

### 本轮施工

- 新增 `src/features/notebook/DragPreview.tsx`，使用只读 CodeMirror 编辑器复用现有 Markdown 语法树和预览装饰，拖拽时继续渲染图片、附件、节点链接及格式化文本。
- 拖拽覆盖层按 `relativeDepth` 计算缩进，并移除几何快照中的源行绝对左内边距；行宽和行间距仍沿用源布局，避免子树预览在拖拽过程发生层级偏移。
- 拖拽预览内容禁用交互，避免覆盖层中的链接、图片或附件控件抢占拖拽事件。
- `src/features/notebook/DragPreview.test.tsx` 覆盖相对层级、粗体/删除线标记、隐藏 Markdown 控制符和远程图片预览；`docs/PROJECT_INDEX.md` 已补充实现与测试索引。README 等待用户验收后更新。

### 验证

- 定向测试通过：3 个文件/59 条测试，覆盖相对层级、Markdown 控制符隐藏、格式标记和图片预览。
- 完整 `npm run check` 通过：模块边界、性能 TypeScript、44 个测试文件/335 条测试、生产构建、Rust 格式和 Rust 编译均正常；Vite 仍只有既有 editor chunk 超过 500 kB 的提示。
- 浏览器冒烟确认本地页面与现有编辑器图片预览正常；拖拽覆盖层的 Live Preview、相对层级和几何快照由组件/定向回归覆盖。本轮未更新 README，等待用户验收。
...
- 需求：侧栏不使用图形 Logo 或副标题，仅展示产品名称。
- 实现：移除侧栏“枝”字标记及“本地优先工作区”副标题，品牌区改为单行“爱思记”；同步清理无用样式并提升品牌文字层级。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：侧栏导航收敛

- 需求：移除侧栏主页和搜索按钮，点击“爱思记”回到主页；回收站移动到设置按钮上方。
- 实现：品牌文字改为可访问按钮并导航到 `home`；主导航仅保留今天和所有笔记；底部导航按回收站、设置排序。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：侧栏今日与快速记录合并

- 需求：将“快速记录”与“今天”合并为同一行，快速记录操作位于右侧，取消圆角。
- 实现：新增无圆角的今日/快速记录相邻操作行；“今天”保留导航选中态，快速记录保留独立点击行为；主导航继续仅保留所有笔记。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：内层滚动条点击隔离

- 问题：Canvas 卡片内部编辑区的滚动条按下仍会命中卡片或笔记内容。
- 原因：此前只检查笔记页外层滚动容器，未沿事件目标向上识别卡片内部的可滚动祖先。
- 实现：滚动条命中判断改为遍历事件目标至笔记容器的祖先链，覆盖主笔记滚动条与卡片内部编辑区滚动条；补充内层滚动容器单元测试。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：侧栏按钮间距收紧

- 需求：侧栏按钮连续排列，修复“今天”与“所有笔记”之间的额外间距。
- 实现：移除今日/快速记录合并行的底部边距，使其与所有笔记、回收站和设置无间距衔接；保留品牌区底部留白。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：快速记录弱化

- 需求：降低合并行中“快速记录”按钮的视觉强调。
- 实现：移除侧栏快速记录按钮的绿色实底，改为透明底与弱文字色，仅悬停时显示浅背景；点击行为和无圆角布局保持不变。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录按钮细节调整

- 需求：移除快速记录按钮的加号，并让按钮默认显示背景。
- 实现：删除 `Plus` 图标及导入；按钮默认使用浅灰背景，悬停切换为淡绿色背景，保持无圆角和右侧布局。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：侧栏记录入口视觉收敛

- 需求：优化快速记录入口的视觉比例，避免与“今天”并列时显得突兀。
- 实现：文案缩短为“记录”；保持右侧紧凑宽度，增加淡分隔线，默认使用接近侧栏的浅灰背景，悬停仅提升背景和文字对比度。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：侧栏宽度与记录文案调整

- 需求：恢复“快速记录”文案，并收窄侧栏。
- 实现：按钮文案恢复为“快速记录”；侧栏宽度由 `252px` 收窄为 `224px`，同步更新关闭状态位移值，合并行保持可用宽度。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录默认背景统一

- 需求：移除快速记录按钮单独的默认背景，避免侧栏出现突兀色块。
- 实现：快速记录按钮默认改为透明背景，与其他侧栏按钮统一；保留淡分隔线和悬停背景反馈。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录图标统一

- 需求：为快速记录按钮补充与侧栏导航一致的图标，改善并列布局协调性。
- 实现：使用 Lucide `PenLine` 线性图标，尺寸与其他侧栏图标一致；保留透明背景、无圆角和右侧位置。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录图标缩小

- 需求：快速记录图标视觉尺寸偏大。
- 实现：将 `PenLine` 图标由 `17px` 调整为 `15px`，保持按钮高度和布局不变。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录悬停主题色

- 需求：快速记录悬停时使用元素主题色反馈，不改变背景。
- 实现：悬停状态保持透明背景，文字与 `PenLine` 图标统一切换为 `var(--leaf)`。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：今日与快速记录等宽

- 需求：修正今天按钮与快速记录按钮左宽右窄的布局观感。
- 实现：合并行两侧按钮统一使用 `flex: 1` 等宽布局；保留今天左对齐、快速记录居中及中间分隔线。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：今日与快速记录层级调整

- 需求：优化等宽双按钮造成的视觉割裂，使今天保持主导航层级、快速记录作为右侧辅助动作。
- 实现：快速记录改为按内容自适应的紧凑宽度，移除中间分隔线和独立色块；今天继续占据剩余空间，快速记录 hover 仅切换主题色。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：快速记录悬停反馈统一

- 需求：修正快速记录仅元素变色导致与今天按钮 hover 反馈不协调的问题。
- 实现：快速记录 hover 改用侧栏统一的浅背景与普通前景色；默认仍保持透明背景。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：今日记录操作行统一背景

- 需求：快速记录无默认背景时与今天并列显得松散，同时避免单独色块突兀。
- 实现：为“今天 + 快速记录”整个操作行增加统一浅背景；两个按钮保持透明，hover 继续分别显示统一的侧栏浅背景反馈，今天选中态覆盖为主题浅色。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：滚动条控件与设置页间距修正

- 需求：移除全局滚动条上下控制按钮，并修复设置页滚动条贴近内容的问题。
- 实现：隐藏 WebKit 滚动条上下按钮及占位尺寸；设置页滚动容器增加右侧内边距，使滚动条与设置卡片内容保持间距。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：设置页滚动条位置与方向按钮修正

- 问题：滚动条方向按钮仍可见，设置页滚动条停留在内容列右侧而非页面最右侧。
- 实现：补充 WebKit 垂直/水平滚动条按钮的方向性伪元素隐藏规则；新增全宽设置页滚动容器，设置内容保持居中窄列，滚动条由全宽容器统一承载。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。滚动条按钮规则随后补充了 WebKit `single-button/start/end` 组合覆盖，并强制隐藏方向性伪元素。

## 2026-09-04：滚动条按钮规则兼容性修正

- 问题：Windows WebView2 中滚动条上下按钮仍可见。
- 原因：方向性伪元素规则混入 WebKit 不稳定的 `start/end` 组合，可能使整个选择器列表失效。
- 实现：收敛为基础 `::-webkit-scrollbar-button` 与标准方向组合，并强制清除按钮尺寸、边框和背景。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：WebView2 原生滚动条按钮修正

- 问题：Windows/WebView2 原生滚动条两端上下按钮仍可见，之前的 WebKit 按钮规则未生效。
- 原因：全局 `scrollbar-width`/`scrollbar-color` 标准属性可能使 Chromium 优先使用原生滚动条，覆盖 `::-webkit-scrollbar-button` 定制。
- 实现：移除这两个标准属性，统一使用 WebKit 滚动条样式，并同时覆盖应用壳和其滚动子元素的上下/左右按钮。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：滚动条可见宽度调整

- 问题：笔记页和设置页滚动条可见部分过细。
- 原因：轨道为 `8px`，滑块两侧各有 `2px` 透明边框，实际可见宽度约为 `4px`。
- 实现：统一将滚动条轨道调整为 `10px`，滑块透明边框调整为 `1px`，实际可见宽度约为 `8px`；上下控制按钮隐藏规则保持不变。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：滚动条交互粗细

- 需求：滚动条默认保持细样式，悬停或拖动控制时变粗。
- 实现：保持 `10px` 轨道占位避免布局跳动；滑块默认使用 `3px` 透明边框，实际可见约 `4px`；悬停或 active 拖动时收窄为 `1px` 边框，实际可见约 `8px` 并提升对比度。
- 验证：`npm run check` 通过（模块边界、性能类型、45 个测试文件/343 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：滚动条拖动焦点保持

- 问题：按下或拖动原生滚动条后，CodeMirror 编辑器的光标焦点消失。
- 根因：滚动条点击隔离只在事件处理层提前返回，未取消 `pointerdown` 的浏览器默认焦点转移；因此编辑器会收到 `blur`，Markdown 源码模式等失焦语义也会执行。
- 实现：在滚动条命中的 `pointerdown` 捕获阶段直接 `preventDefault()`，前置取消焦点转移并继续放行原生滚动条拖动；画布点击与节点范围选择仍在同一命中处提前退出，避免事件穿透。
- 验证：真实 Chromium 浏览器验证内层滚动条拖动仍会滚动且编辑器焦点不丢失；补充滚动条命中与 NotebookPanel 回归测试；`npm run check` 通过（模块边界、性能类型、46 个测试文件/348 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：滚动条指针样式

- 问题：鼠标位于滚动条时仍继承画布的 `text` 指针，无法直观看出滚动条的可拖动区域。
- 实现：为 WebKit 滚动条轨道设置 `default` 指针；滑块设置 `grab`，按住滑块时切换为 `grabbing`；滚动条尺寸、颜色和焦点隔离逻辑保持不变。
- 验证：真实 Chromium 页面确认滚动条本体/轨道为 `default`、滑块为 `grab`；`npm run check` 通过（模块边界、性能类型、46 个测试文件/348 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：日期文本点击不进入页面

- 需求：保留点击日期圆点进入日期页的能力，移除点击日期节点文本直接进入的路径。
- 实现：删除画布坐标命中日期行时的 `enterNode` 调用；日期文本及其行内空白命中后不改变当前页面，日期圆点按钮仍由 `TreeRow` 保留原有进入行为。
- 验证：补充 NotebookPanel 回归测试，覆盖日期文本不进入与日期圆点仍可进入；待运行 `npm run check`。

## 2026-09-04：日期节点圆点样式统一

- 需求：日期节点的圆点改为与普通节点一致，同时保留圆点点击进入日期页的行为。
- 实现：移除日期圆点专属的大尺寸、叶绿色样式；日期与普通节点统一使用 `6px` 圆点、相同描边和默认前景色。点击处理仍由原有节点圆点按钮负责。
- 验证：补充 NotebookPanel 圆点几何属性回归测试；待运行 `npm run check`。

## 2026-09-04：Supertag 跟随节点内容

- 需求：节点 supertag 芯片紧跟节点内容显示，不再在整行右侧对齐。
- 原因：`InlineEditor` 外层 flex 让编辑器使用 `flex: 1 1 auto` 占满剩余宽度，芯片自然被推到行尾。
- 实现：编辑器改为按内容自然占位并限制在行宽内；编辑器外层允许芯片在空间不足时换行，芯片保持紧邻内容的顺序和原有移除交互。
- 验证：真实 Chromium 页面确认编辑器按内容自然收缩、外层允许换行；`npm run check` 通过（模块边界、性能类型、46 个测试文件/350 条测试、生产构建、Rust 格式与编译）；构建保留既有 editor chunk 超过 500 kB 的提示。

## 2026-09-04：滚动条点击事件隔离

- 问题：按下原生滚动条时，笔记页会同时触发页面内容点击或节点选择。
- 原因：WebView2 可能将滚动条按下事件冒泡到滚动容器，画布落点和节点范围选择未识别滚动条命中区域。
- 实现：新增统一滚动条命中判断；笔记画布点击处理和节点选择捕获处理在滚动条区域提前退出，避免滚动操作穿透到页面内容。
- 验证：待运行前端测试与生产构建。

## 2026-09-04：已验收交互同步至项目说明

- 结果：用户确认本轮侧栏、滚动条、日期节点与 Supertag 交互改动验收无误，同意提交。
- 文档：README 补充日期节点仅由圆点进入、日期圆点样式统一、Supertag 紧跟内容，以及滚动条焦点保持、点击隔离和指针反馈说明。
- 验证：此前 `npm run check` 已通过（模块边界、性能类型、46 个测试文件/350 条测试、生产构建、Rust 格式与编译）；本次 README 与日志更新通过 `git diff --check`。
