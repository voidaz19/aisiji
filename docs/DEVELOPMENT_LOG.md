...
## 2026-09-02：Canvas Supertag 首版

### 用户确认与范围

- 用户确认 Canvas 采用 Tana 风格 supertag，而非独立节点类型。
- 首版明确不做自由卡片、节点连线和拖拽定位；因此不引入坐标、连线或专属画布持久化模型。

### 施工计划

- 为 `NodeRecord` 增加可组合的稳定 supertag ID 元数据，并以纯领域函数维护 Canvas 标签。
- 将带 Canvas 标签的内容节点呈现为其直接子节点的自动网格；保留大纲、节点身份及所有现有树操作。
- 新增领域/UI 回归测试，完成后执行 `npm run check`。README 依项目约定等待用户验收后更新。

### 实现

- `src/domain/supertags.ts` 定义内建 Canvas 标签与通用增删/查询纯函数；标签只附着于内容节点，并维护节点 revision。
- `src/features/canvas/CanvasPanel.tsx` 将 Canvas 节点的直接子节点按原树顺序呈现为响应式自动网格；卡片内容沿用现有 `InlineEditor`，新建卡片沿用 `createChild`。
- 普通内容页标题区增加“添加 Canvas”入口；Canvas 页提供移除标签入口。移除标签不会删除或迁移任何子节点。
- `docs/CANVAS_SUPERTAG_SPEC.md` 固化数据、交互和非目标；`docs/PROJECT_INDEX.md` 已补充规格与实现索引。README 等待用户实际验收后再更新。

### 验证

- 定向测试通过：Canvas 领域、Canvas UI、笔记面板与 Store 共 4 个测试文件/81 条测试。
- 首次完整检查在生产构建阶段发现 Canvas 组件残留一个未使用订阅；已移除并重新执行完整检查。
- 架构复核发现静态导入 Canvas 会绕过既有笔记模块懒加载边界；已将 Canvas 与 Notebook 组成同一按需加载组。定向复跑为 5 个文件/83 条测试，生产构建确认 Canvas 保持独立 chunk。
- 最终 `npm run check` 通过：模块边界、性能 TypeScript、43 个测试文件/327 条测试、生产构建、Rust 格式和 Rust 编译均正常；仍只有既有 editor chunk 超过 500 kB 提示。
- 浏览器实测通过：添加/移除 Canvas 标签、新建并编辑卡片正常；1280×720 与 390×844 下均无横向溢出，窄屏卡片自动收敛为单列。

## 2026-09-02：Canvas 网格长内容约束

### 用户反馈与处理

- 用户指出当前 Canvas 视图不是绘画白板，并反馈部分节点内容过长，导致自动网格卡片撑高、扫描困难。
- 本轮只修复现有自动网格的长内容表现，不改变 Canvas supertag 方向，也不提前建设绘画画布架构。
- 卡片增加固定最大高度，编辑区域在卡片内滚动；Markdown 原文和节点数据不截断。需要完整上下文时仍可进入节点普通页面查看。
- 浏览器回归验证通过：长内容卡片高度约 176px、编辑区高度约 132px，超出文本在卡片内部滚动；桌面与窄屏均无横向溢出。

## 2026-09-02：Canvas 卡片背景强化

### 用户反馈与处理

- 用户指出 Canvas 卡片与页面底色接近，节点边界不清晰。
- 卡片改用浅叶色混合背景、低强度边框和轻阴影；悬停或编辑时进一步提高背景与边框对比度，不改变卡片尺寸、滚动和数据语义。
- 用户进一步确认不使用阴影；已移除默认和悬停阴影，仅保留背景与边框对比度。
- 用户反馈背景颜色偏重；已将默认背景调整为 10% 叶色混合、悬停/编辑态调整为 18% 叶色混合，保持轻量识别度。
...
## 2026-09-02：Canvas 节点树内局部视图

- 需求：Canvas supertag 节点不只在作为页面根节点时显示网格，在普通节点树中展开时也局部显示 Canvas。
- 决策：保留 Canvas 父节点标题行；局部网格替代其普通可见子树，避免直接子节点重复，并通过折叠状态统一控制显隐；搜索与回收站不改变原有行视图。
- 实现：抽取根视图与局部视图共用的 `CanvasCardGrid`；树布局纯函数标记局部 Canvas 并跳过其后代行；局部网格随父行共同参与窗口化高度测量和拖拽几何计算。
- 验证：定向回归通过（3 个文件/61 条测试）；完整 `npm run check` 通过（模块边界、性能类型、43 个测试文件/331 条测试、生产构建、Rust 格式与编译）。仍只有既有 editor chunk 超过 500 kB 提示。
- 浏览器实测：1280×720 下局部 Canvas 正常嵌入父节点行，长卡片保持 176px 高度且无阴影；390×844 下卡片收敛为单列，文档宽度等于视口宽度，无横向溢出；折叠后局部卡片清零，重新展开恢复。
- README：依项目约定，等待用户确认本轮功能有效后再更新。

## 2026-09-02：设置页布局调试入口

- 需求：将已有 URL 调试参数和笔记页内的布局调试框选项加入设置页。
- 实现：AppShell 统一持有布局调试启用状态及各框选项；SettingsPanel 提供启用开关并复用 `LayoutDebugPanel`；NotebookPanel 接收共享选项，切换页面后保持当前选择。
- 兼容：设置页单独挂载时使用本地状态，测试和独立组件调用仍可直接操作。
- 验证：边界检查和生产构建通过；设置页与 AppShell 定向测试正在修复后复跑。README 等待用户验收后再更新。

## 2026-09-02：Canvas 网格同行等高

- 问题：卡片按内容使用不同高度，而 CSS Grid 的下一行仍从本行最高卡片之后开始，短卡片下方因此出现大块视觉空隙。
- 处理：根 Canvas 与树内局部 Canvas 共用的网格改为同行拉伸等高；不采用全局固定高度，窄屏单列和内容较短的独立行仍保持自然高度。
- 验证：浏览器 1280px 实测同一行卡片由 `116/176px` 对齐为 `176/176px`；390px 单列下仍为 `116/176px` 自然高度且无横向溢出。完整 `npm run check` 通过（43 个测试文件/331 条测试、生产构建、Rust 格式与编译），仍只有既有 editor chunk 超过 500 kB 提示。

## 2026-09-02：局部 Canvas 层级线

- 问题：局部 Canvas 用网格替代普通子节点行后，既有层级线测量无法识别这块可见子树，导致父节点与卡片的层级关系在视觉上中断。
- 处理：层级线测量统一识别局部 Canvas，以网格真实底部作为子树终点；线条保持普通树的颜色、命中宽度、键盘能力及“折叠或展开下一级节点”行为，不增加 Canvas 专用操作。
- 一致性修正：曾尝试让局部 Canvas 层级线直接折叠 Canvas 父节点，用户指出这会破坏普通层级线的一致性；该特殊分支已完全移除。
- 验证：待完成界面回归、浏览器测量和完整 `npm run check`。
