...
此前源码审查确认拖拽时会关闭虚拟化；本轮已修正为拖拽期间继续使用虚拟器，1k/10k 拖拽专项结果均为 37 个 DOM 行，结构红线已消除。当前剩余失败继续驱动 `docs/PERFORMANCE_AUDIT_2026-08-02.md` 中的 Store 失效边界和增量持久化施工，不得通过放宽预算消除。
