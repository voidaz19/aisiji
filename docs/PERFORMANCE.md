...
| 层级 | 实现 | 覆盖范围 |
| --- | --- | --- |
| 纯逻辑基准 | `performance/domain.perf.test.ts`、专用 Vitest 配置 | 1k/10k 可见树、10k 搜索、1k 布局行、10k 拖拽槽位和 10k 深层子树矩形计算 |
| Chromium 交互 | `performance/browser.perf.spec.ts`、Playwright | 生产构建启动、10k 大纲、1k 展开、100KB/1MB Markdown、10k+100KB 组合输入、160 节点范围选择、节点切换、40/160/1k/10k 拖拽、Live Preview 构建/扫描范围、输入分段、重渲染/拓扑计数、长任务、帧间隔、DOM 数量和 JS 堆增长 |
...
- 160/1k/10k 拖拽在按住指针期间继续检查 DOM 行数上限，禁止通过关闭虚拟化获得拖拽布局。
- 160 节点页面覆盖此前 40 与 1k 场景之间的窗口化盲区；节点范围选择要求零 Long Task，选择与拖拽期间 DOM 行均不得超过 100。
- 10k 拖拽槽位和深层子树矩形分别设置纯逻辑 P95 预算，真实 Chromium 同时限制 10k 拖拽预览出现时间；禁止只约束 DOM 数量而留下同步平方级计算。
...
2026-08-27 用户复测后澄清，卡顿与单个节点 Markdown 长度无关，而是页面中存在较多节点时的节点范围选择和节点拖拽。窗口化门槛已由 200 行下调到 80 行，并新增 160 节点短文本夹具：选择和拖拽均只挂载 37 行，范围选择无 Long Task；节点拖选期间不再逐帧执行全树的批量缩进/提升命令预览，命令能力只在选区完成、菜单可用后计算。

同日后续修正了极端规模拖拽启动路径：`createTreeDropSlots` 原先对每个槽位线性查重，并重复复制/扫描移动节点的同级列表；深层子树矩形也会为每个根重复向后扫描。两处现已改为集合/一次性位置索引和栈式单次扫描。10k 槽位 P95 为 13.13ms，10k 深层子树矩形 P95 为 0.35ms；Chromium 10k 拖拽启动由约 4.18s 降至单场景 227ms、完整专项矩阵 608ms，37 个 DOM 行保持不变并通过 1000ms/100 行硬预算。
