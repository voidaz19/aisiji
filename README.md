...
- 全局树形大纲、日期节点和今天入口；日期文本保持普通内容点击语义，仅节点圆点用于进入日期页，日期圆点与普通节点使用一致样式
- 页面导航历史：顶部前进/后退按钮与 `Alt+左/右方向键` 可在主页、设置、笔记视图和具体笔记位置之间切换
- 节点进入、折叠、回车拆分、键盘缩进/提升和整棵子树拖拽；空节点回车在下方连续创建，提升不会越过当前页面根
- 子树拖拽预览保留原始缩进，Canvas 节点保持卡片网格视图，Markdown/图片继续使用 Live Preview；层级线随拖拽覆盖层移动，并在松手时与节点同步平滑回位和恢复透明度
...
- 卡片视图 Supertag：在节点内容的词边界输入 `#`，从补全浮窗选择 `#卡片视图`；可移除标签紧跟节点内容并在空间不足时换行，带标签节点自动以直接子节点卡片网格呈现，根节点和树内局部视图均适用
- 中等与大型节点页保持窗口化渲染；节点范围拖选期间不执行菜单专用的全树命令预演，节点数量增加时仍限制实际挂载行数
- 设置页提供一次性调试样例生成：可生成用于节点选择、拖拽、层级、折叠和 Markdown 预览的固定测试节点，已有样例时不会重复生成
- 文本与节点选区浮动菜单：完成选择后在对象上方显示；文本支持常用格式、复制和剪切，节点支持复制、剪切、删除以及当前可执行的提升/缩进操作，复制后提供明确反馈
- 统一的桌面与浏览器控件视觉：画布、按钮、输入框、文本选区和滚动条使用应用样式；滚动条悬停与拖动有明确的粗细和指针反馈，操作滚动条不会穿透到画布或转移编辑焦点
- 语法树驱动的 Markdown Live Preview：H1-H6、引用、待办、粗体、斜体、删除线、高亮、行内代码和分隔线；长文按可见范围更新预览装饰
- 统一的插入与格式菜单：单独按下并松开 `Ctrl` 可在当前光标附近打开，搜索并执行格式、结构、节点链接和文件操作；Markdown 快捷输入仍可直接使用
...
性能回归使用固定的 160/1k/10k 工作区与 100KB/1MB Markdown 夹具，覆盖输入分段、Live Preview 扫描范围、Long Task、虚拟化 DOM、节点范围选择、节点切换和拖拽：

```powershell
npm run test:perf
```

当前性能红线与重大重做边界见 [性能测试规范](docs/PERFORMANCE.md) 和 [2026-08-02 性能架构审查](docs/PERFORMANCE_AUDIT_2026-08-02.md)。中等规模节点页的选择与拖拽窗口化已通过用户验收，10k 节点拖拽启动也已有独立预算；Store 失效边界、增量持久化和启动生命周期仍按路线继续施工。性能测试失败表示已记录的问题仍存在，不应通过放宽预算隐藏。
